...
public class PeerConnectionClient {
    ...
    public static class PeerConnectionParameters {
        ...
        public PeerConnectionParameters(boolean videoCallEnabled, boolean loopback, boolean tracing,
                                        ...
                                        boolean enableLevelControl, StreamMode streamMode, DataChannelParameters dataChannelParameters) {
    ...
    public PeerConnectionClient(Context appContext, EglBase eglBase,
        ...
        Timber.tag(TAG).d("Preferred video codec: %s", getSdpVideoCodecName(peerConnectionParameters));
        final String fieldTrials = getFieldTrials(peerConnectionParameters);
        executor.execute(() -> {
            Timber.tag(TAG).d("Initialize WebRTC. Field trials: %s", fieldTrials);
    ...
    public void createPeerConnection(final VideoSink localRender, final List<VideoSink> remoteSinks,
        ...
        if (peerConnectionParameters == null) {
            Timber.tag(TAG).e("Creating peer connection without initializing factory.");
            return;
        }
    ...
    AudioDeviceModule createJavaAudioDevice() {
        ...
        AudioRecordErrorCallback audioRecordErrorCallback = new AudioRecordErrorCallback() {
            ...
            public void onWebRtcAudioRecordInitError(String errorMessage) {
                Timber.tag(TAG).e("onWebRtcAudioRecordInitError: %s", errorMessage);
            ...
            public void onWebRtcAudioRecordStartError(
                ...
                Timber.tag(TAG).e("onWebRtcAudioRecordStartError: " + errorCode + ". " + errorMessage);
                reportError(errorMessage);
            }

            @Override
            public void onWebRtcAudioRecordError(String errorMessage) {
                Timber.tag(TAG).e("onWebRtcAudioRecordError: %s", errorMessage);
                reportError(errorMessage);
            }
        };
        AudioTrackErrorCallback audioTrackErrorCallback = new AudioTrackErrorCallback() {
            ...
            public void onWebRtcAudioTrackInitError(String errorMessage) {
                Timber.tag(TAG).e("onWebRtcAudioTrackInitError: %s", errorMessage);
                reportError(errorMessage);
            }
            ...
            public void onWebRtcAudioTrackError(String errorMessage) {
                Timber.tag(TAG).e("onWebRtcAudioTrackError: %s", errorMessage);
                reportError(errorMessage);
            }
        };
        ...
        AudioRecordStateCallback audioRecordStateCallback = new AudioRecordStateCallback() {
            ...
            public void onWebRtcAudioRecordStart() {
                Timber.tag(TAG).i("Audio recording starts");
            }

            @Override
            public void onWebRtcAudioRecordStop() {
                Timber.tag(TAG).i("Audio recording stops");
            }
        };
        ...
        AudioTrackStateCallback audioTrackStateCallback = new AudioTrackStateCallback() {
            ...
            public void onWebRtcAudioTrackStart() {
                Timber.tag(TAG).i("Audio playout starts");
            }

            @Override
            public void onWebRtcAudioTrackStop() {
                Timber.tag(TAG).i("Audio playout stops");
            }
        };
    ...
    private void createMediaConstraintsInternal() {
        ...
        if (peerConnectionParameters.enableLevelControl) {
            Timber.tag(TAG).d("Enabling level control.");
            audioConstraints.mandatory.add(
    ...
    private void createPeerConnectionInternal() {
        if (factory == null || isError) {
            Timber.tag(TAG).e("Peerconnection factory is not created");
            return;
        }
        ...
        if (peerConnectionParameters.aecDump) {
            try {
                ...
                Timber.tag(TAG).e(e, "Can not open aecdump file");
            }
        }
        Timber.tag(TAG).d("Peer connection created.");
    }
    ...
    public void setRemoteDescription(final SessionDescription sdp) {
        executor.execute(() -> {
            ...
            Timber.tag(TAG).d("Set remote SDP.");
    ...
    public void setVideoMaxBitrate(@Nullable final Integer maxBitrateKbps) {
        executor.execute(() -> {
            ...
            Timber.tag(TAG).d("Requested max video bitrate: %s", maxBitrateKbps);
            ...
            if (!localVideoSender.setParameters(parameters)) {
                Timber.tag(TAG).e("RtpSender.setParameters failed.");
            }
            Timber.tag(TAG).d("Configured max video bitrate to: %s", maxBitrateKbps);
        });
    }

    private void reportError(final String errorMessage) {
        Timber.tag(TAG).e("Peerconnection error: %s", errorMessage);
        executor.execute(() -> {
            if (!isError) {
    ...
    private static String getFieldTrials(PeerConnectionParameters peerConnectionParameters) {
        ...
        if (peerConnectionParameters.videoFlexfecEnabled) {
            ...
            Timber.tag(TAG).d("Enable FlexFEC field trial.");
        }
        fieldTrials += VIDEO_VP8_INTEL_HW_ENCODER_FIELDTRIAL;
        if (peerConnectionParameters.disableWebRtcAGCAndHPF) {
            fieldTrials += DISABLE_WEBRTC_AGC_FIELDTRIAL;
            Timber.tag(TAG).d("Disable WebRTC AGC field trial.");
        }
        return fieldTrials;
    }
    ...
    private static String setStartBitrate(
        ...
        if (codecRtpMap == null) {
            Timber.tag(TAG).w("No rtpmap for " + codec + " codec");
            return sdpDescription;
        }
        Timber.tag(TAG).d("Found " + codec + " rtpmap " + codecRtpMap + " at " + lines[rtpmapLineIndex]);
        ...
        for (int i = 0; i < lines.length; i++) {
            Matcher codecMatcher = codecPattern.matcher(lines[i]);
            if (codecMatcher.matches()) {
                Timber.tag(TAG).d("Found " + codec + " " + lines[i]);
                if (isVideoCodec) {
                    lines[i] += "; " + VIDEO_CODEC_PARAM_START_BITRATE + "=" + bitrateKbps;
                } else {
                    lines[i] += "; " + AUDIO_CODEC_PARAM_BITRATE + "=" + (bitrateKbps * 1000);
                }
                Timber.tag(TAG).d("Update remote SDP line: " + lines[i]);
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (!sdpFormatUpdated && i == rtpmapLineIndex) {
                ...
                Timber.tag(TAG).d("Add remote SDP line: " + bitrateSet);
    ...
    String movePayloadTypesToFront(
        ...
        if (origLineParts.size() <= 3) {
            Timber.tag(TAG).e("Wrong SDP media description format: %s", mLine);
            return null;
        }
    ...
    private static String preferCodec(String sdpDescription, String codec, boolean isAudio) {
        ...
        Timber.tag(TAG).d("Change media description from: " + lines[mLineIndex] + " to " + newMLine);
    ...
    private void drainCandidates() {
        if (queuedRemoteCandidates != null) {
            Timber.tag(TAG).d("Add " + queuedRemoteCandidates.size() + " remote candidates");
    ...
    private void switchCameraInternal() {
        if (videoCapturer instanceof CameraVideoCapturer) {
            if (!isVideoCallEnabled() || isError) {
                Timber.tag(TAG).e("Failed to switch camera. Video: " + isVideoCallEnabled() + ". Error : " + isError);
    ...
    private void changeCaptureFormatInternal(int width, int height, int framerate) {
        if (!isVideoCallEnabled() || isError || videoCapturer == null) {
            Timber.tag(TAG).e("Failed to change capture format. Video: " + isVideoCallEnabled()
                    + ". Error : " + isError);
            return;
        }
    ...
    private class PCObserver implements PeerConnection.Observer {
        ...
        public void onSignalingChange(PeerConnection.SignalingState newState) {
            Timber.tag(TAG).d("SignalingState: %s", newState);
        }

        @Override
        public void onIceConnectionChange(final PeerConnection.IceConnectionState newState) {
            executor.execute(() -> {
                Timber.tag(TAG).d("IceConnectionState: %s", newState);
        ...
        public void onConnectionChange(final PeerConnection.PeerConnectionState newState) {
            executor.execute(() -> {
                Timber.tag(TAG).d("PeerConnectionState: %s", newState);
        ...
        public void onIceGatheringChange(PeerConnection.IceGatheringState newState) {
            Timber.tag(TAG).d("IceGatheringState: %s", newState);
        }

        @Override
        public void onIceConnectionReceivingChange(boolean receiving) {
            Timber.tag(TAG).d("IceConnectionReceiving changed to %s", receiving);
        }

        @Override
        public void onSelectedCandidatePairChanged(CandidatePairChangeEvent event) {
            Timber.tag(TAG).d("Selected candidate pair changed because: %s", event);
        }
        ...
        public void onDataChannel(final DataChannel dc) {
            Timber.tag(TAG).d("New Data channel %s", dc.label());
            if (!dataChannelEnabled) {
                return;
            }
            dc.registerObserver(new DataChannel.Observer() {
                ...
                public void onBufferedAmountChange(long previousAmount) {
                    Timber.tag(TAG).d("Data channel buffered amount changed: " + dc.label() + ": " + dc.state());
                }

                @Override
                public void onStateChange() {
                    Timber.tag(TAG).d("Data channel state changed: " + dc.label() + ": " + dc.state());
                }

                @Override
                public void onMessage(final DataChannel.Buffer buffer) {
                    if (buffer.binary) {
                        Timber.tag(TAG).d("Received binary msg over %s", dc);
                        return;
                    }
                    ...
                    Timber.tag(TAG).d("Got msg: " + strData + " over " + dc);
                }
            });
        }
    ...
    private class SDPObserver implements SdpObserver {
        ...
        public void onCreateSuccess(final SessionDescription origSdp) {
            ...
            executor.execute(() -> {
                if (peerConnection != null && !isError) {
                    Timber.tag(TAG).d("Set local SDP from %s", sdp.type);
            ...
            executor.execute(() -> {
                ...
                if (isInitiator) {
                    ...
                    if (peerConnection.getRemoteDescription() == null) {
                        ...
                        Timber.tag(TAG).d("Local SDP set succesfully");
                        events.onLocalDescription(localSdp);
                    } else {
                        // We've just set remote description, so drain remote
                        // and send local ICE candidates.
                        Timber.tag(TAG).d("Remote SDP set succesfully");
                        drainCandidates();
                    }
                } else {
